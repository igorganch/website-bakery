...
$(document).ready(function(){
    ...
    */
    form.addEventListener("submit",onsubmit);
     ...
     function removeProduct(e){
         ...
         var urlid = $("#form-photo-details").prop("action");
  ...
  function load(){
        var path = $("#form-photo-details").prop("action");
        console.log(path);
        ...
        $.ajax({
            ...
            var price = result[0].Stock.price;
...
async function onsubmit(e){
       ...
       for(let i = 0 ; i < $(".photo-upload-pic").length; i++){
        ...
        $.ajax({
            type : "POST",
            url : $("#form-photo-details").prop("action"),
            data : formData,
            processData: false,
            contentType: false,
            success : function(data){
